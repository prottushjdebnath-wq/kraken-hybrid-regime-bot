...
# Futures / Derivatives Configuration
ENABLE_FUTURES = os.environ.get("ENABLE_FUTURES", "false").strip().lower() in ("true", "1", "yes")
FUTURES_MAX_LEVERAGE = float(os.environ.get("FUTURES_MAX_LEVERAGE", "5.0"))
FUTURES_MAX_PORTFOLIO_MARGIN_PCT = float(os.environ.get("FUTURES_MAX_PORTFOLIO_MARGIN_PCT", "0.35"))

# Trade Quality Grading & Anti-Overtrading Controls
# Grades: A (highest conviction) -> D (lowest). Only trades scoring at/above
# MIN_GRADE_TO_TRADE are allowed to execute, regardless of free capacity.
GRADE_THRESHOLDS = {"A": 0.75, "B": 0.55, "C": 0.40, "D": 0.0}
MIN_GRADE_TO_TRADE = os.environ.get("MIN_GRADE_TO_TRADE", "B").strip().upper()
if MIN_GRADE_TO_TRADE not in GRADE_THRESHOLDS:
    MIN_GRADE_TO_TRADE = "B"

# A signal must re-appear above threshold for this many consecutive scan
# cycles before the bot will act on it. This is the primary defense against
# "to-and-fro" (flip-flop) trading on noisy/borderline setups.
MIN_SIGNAL_CONFIRMATIONS = int(os.environ.get("MIN_SIGNAL_CONFIRMATIONS", "2"))

# After a position on a given market closes, that market is frozen from
# re-entry for this many minutes, win or loss, to stop immediate re-chasing.
...
# market_type_by_venue: "SPOT" or "FUTURES" — drives leverage caps, margin
# budget caps and dashboard tagging. Populated as each venue is created.
market_type_by_venue = {}
...
if "binance" in ENABLED_EXCHANGES:
    ...
    market_type_by_venue['binance'] = 'SPOT'

    if ENABLE_FUTURES:
        bf_params = {'enableRateLimit': True, 'options': {'defaultType': 'future'}}
        if BINANCE_API_KEY and BINANCE_SECRET_KEY:
            bf_params['apiKey'] = BINANCE_API_KEY
            bf_params['secret'] = BINANCE_SECRET_KEY
        bf_client = ccxt.binance(bf_params)
        if BINANCE_TESTNET:
            bf_client.set_sandbox_mode(True)
            print("🧪 [BINANCE FUTURES] Sandbox/Testnet routing engaged.")
        exchanges['binance_futures'] = bf_client
        market_type_by_venue['binance_futures'] = 'FUTURES'
...
if "kraken" in ENABLED_EXCHANGES:
    ...
    market_type_by_venue['kraken'] = 'SPOT'

    if ENABLE_FUTURES:
        try:
            kf_params = {'enableRateLimit': True}
            if KRAKEN_API_KEY and KRAKEN_SECRET_KEY:
                kf_params['apiKey'] = KRAKEN_API_KEY
                kf_params['secret'] = KRAKEN_SECRET_KEY
            kf_client = ccxt.krakenfutures(kf_params)
            exchanges['kraken_futures'] = kf_client
            market_type_by_venue['kraken_futures'] = 'FUTURES'
        except AttributeError:
            print("⚠️ Installed ccxt version has no krakenfutures class — skipping Kraken Futures venue. Upgrade ccxt to enable it.")

print(f"🏛️ Initialized {len(exchanges)} execution venues: {list(exchanges.keys())}")
if ENABLE_FUTURES:
    print(f"📐 Futures venues capped at {FUTURES_MAX_LEVERAGE}x leverage / {FUTURES_MAX_PORTFOLIO_MARGIN_PCT*100:.0f}% portfolio margin budget.")
...
# signal_confirmations schema: { "BINANCE:BTC/USDT": {"regime": "SWING", "count": 2, "score": 0.61} }
# Tracks how many consecutive scan cycles a setup has held above threshold.
# A trade only fires once count >= MIN_SIGNAL_CONFIRMATIONS, which is the
# core defense against flip-flop / to-and-fro entries on noisy signals.
signal_confirmations = {}

# symbol_cooldown_until schema: { "BINANCE:BTC/USDT": iso_timestamp_string }
# A market already traded recently is frozen from re-entry until this time,
# regardless of signal quality, to stop immediate re-chasing after a close.
...
def get_status_snapshot():
    with state_lock:
        ...
        margin_used_spot = sum(
            p["margin_allocated"] for p in positions.values()
            if p.get("position_active", False) and market_type_by_venue.get(p.get("exchange"), "SPOT") == "SPOT"
        )
        margin_used_futures = sum(
            p["margin_allocated"] for p in positions.values()
            if p.get("position_active", False) and market_type_by_venue.get(p.get("exchange"), "SPOT") == "FUTURES"
        )
        active_signal_watch = {
            k: v for k, v in signal_confirmations.items()
            if v.get("count", 0) < MIN_SIGNAL_CONFIRMATIONS
        }
        cooling_down = {
            k: v for k, v in symbol_cooldown_until.items()
            if datetime.now(timezone.utc) < datetime.fromisoformat(v)
        }
        ...
        snapshot.update({
            ...
            "market_type_by_venue": dict(market_type_by_venue),
            "asset_allocation": {
                "margin_used_spot": round(margin_used_spot, 2),
                "margin_used_futures": round(margin_used_futures, 2),
                "spot_budget_pct": MAX_PORTFOLIO_MARGIN_PCT,
                "futures_budget_pct": FUTURES_MAX_PORTFOLIO_MARGIN_PCT if ENABLE_FUTURES else 0.0,
            ...
            "risk_manager": {
                ...
                "signals_building_conviction": active_signal_watch,
                "symbols_in_cooldown": cooling_down
            },
            "bot_parameters": {
                "risk_pct_per_trade": RISK_PCT_PER_TRADE,
                "rr_ratio": RR_RATIO,
                ...
                "max_leverage": MAX_LEVERAGE,
                "futures_max_leverage": FUTURES_MAX_LEVERAGE if ENABLE_FUTURES else None,
    ...
    try:
        ...
        with state_lock:
            ...
            for key, p in saved_positions.items():
                ...
                grade = p.get('grade', grade_trade_quality(conf))
                mtype = p.get('market_type', market_type_by_venue.get(ex_name.lower(), 'SPOT'))
...
def log_trade_to_ledger(timestamp, exchange_name, symbol, order_id, side, regime, price, amount, stop_loss, take_profit, leverage, status, market_type=None, grade=None, pnl=None):
    trade_record = {
        'Timestamp': timestamp,
        'Exchange': exchange_name.upper(),
        'MarketType': market_type or market_type_by_venue.get(exchange_name, 'SPOT'),
        ...
        'PnL': round(float(pnl), 2) if pnl is not None else '',
        'Status': status
    }
...
# =====================================================================
def _default_symbols_for_venue(client, ex_name, base_assets):
    """Builds a safe fallback symbol list correct for spot vs futures/swap
    symbol notation on the given venue (e.g. 'BTC/USDT' vs 'BTC/USDT:USDT')."""
    is_futures_venue = market_type_by_venue.get(ex_name) == 'FUTURES'
    out = []
    try:
    ...
    for base in base_assets:
        candidate_plain = f"{base}/{SCAN_QUOTE_CURRENCY}"
        candidate_swap = f"{base}/{SCAN_QUOTE_CURRENCY}:{SCAN_QUOTE_CURRENCY}"
        if is_futures_venue:
            if client.markets and candidate_swap in client.markets:
                out.append(candidate_swap)
            elif client.markets and candidate_plain in client.markets:
                out.append(candidate_plain)
        else:
            out.append(candidate_plain)
    return out
    ...
    for ex_name, client in exchanges.items():
        ...
        try:
            ...
            for symbol, ticker in tickers.items():
                market_meta = client.markets.get(symbol)
                if not market_meta or not market_meta.get('active', True):
                    continue

                # Robust quote-currency match via market metadata (handles
                # both plain spot symbols "BTC/USDT" and contract symbols
                # like "BTC/USDT:USDT" used on futures/swap venues).
                if market_meta.get('quote') != SCAN_QUOTE_CURRENCY:
                    continue

                if is_futures_venue:
                    if not (market_meta.get('swap') or market_meta.get('future') or market_meta.get('contract')):
                        continue
                    if not market_meta.get('linear', True):
                        continue  # skip inverse contracts, keep sizing simple
                else:
                    if not market_meta.get('spot', True):
                        continue

                base = market_meta.get('base', symbol.split('/')[0])
            ...
            if not top_pairs:
                top_pairs = _default_symbols_for_venue(client, ex_name, ['BTC', 'ETH', 'SOL'])

            universe[ex_name] = list(dict.fromkeys(top_pairs + manual_syms))

        except Exception as e:
            print(f"⚠️ Discovery warning for {ex_name.upper()}: {e}")
            fallback = _default_symbols_for_venue(client, ex_name, ['BTC', 'ETH'])
...
def grade_trade_quality(score):
    """Maps a raw 0.0-1.0 opportunity score to a letter grade A-D."""
...
def grade_passes_minimum(grade):
    order = ["D", "C", "B", "A"]
    return order.index(grade) >= order.index(MIN_GRADE_TO_TRADE)

def register_signal_and_check_confirmed(composite_key, regime, score):
    """Anti-overtrading gate #1: requires the SAME regime/score-tier signal
    to reappear on MIN_SIGNAL_CONFIRMATIONS consecutive scan cycles before
    it is considered tradeable. A regime flip or a drop below grade resets
    the counter. Returns (is_confirmed: bool, confirmations_so_far: int)."""
    grade = grade_trade_quality(score)
    with state_lock:
        prior = signal_confirmations.get(composite_key)
        if prior and prior.get("regime") == regime and prior.get("grade") == grade:
            new_count = prior.get("count", 0) + 1
        else:
            new_count = 1
        signal_confirmations[composite_key] = {"regime": regime, "grade": grade, "score": score, "count": new_count}
    return new_count >= MIN_SIGNAL_CONFIRMATIONS, new_count

def clear_signal_confirmation(composite_key):
    with state_lock:
        signal_confirmations.pop(composite_key, None)

def is_symbol_in_cooldown(composite_key):
    with state_lock:
        until_str = symbol_cooldown_until.get(composite_key)
    if not until_str:
        return False, None
    try:
    ...
    return now < until_dt, until_str

def start_symbol_cooldown(composite_key):
    if SYMBOL_COOLDOWN_MINUTES <= 0:
        return
    from datetime import timedelta
    until_dt = datetime.now(timezone.utc) + timedelta(minutes=SYMBOL_COOLDOWN_MINUTES)
    with state_lock:
        symbol_cooldown_until[composite_key] = until_dt.isoformat()
...
def calculate_dynamic_entry(client, ex_name, symbol, current_price, atr, regime, score):
    is_futures = market_type_by_venue.get(ex_name) == 'FUTURES'
    ...
    venue_max_leverage = FUTURES_MAX_LEVERAGE if is_futures else MAX_LEVERAGE
    effective_max_leverage = min(venue_max_leverage, MAX_LEVERAGE if not is_futures else venue_max_leverage)
    effective_max_leverage = max(MIN_LEVERAGE, effective_max_leverage)
    ...
    # Spot and futures draw from SEPARATE margin budgets so a run of futures
    # signals can never crowd out spot allocation (and vice versa) — this is
    # the "asset allocation" split between the two sleeves.
    with state_lock:
        current_margin_used = sum(
            p["margin_allocated"] for p in positions.values()
            if p.get("position_active", False) and (market_type_by_venue.get(p.get("exchange"), "SPOT") == "FUTURES") == is_futures
        )

    budget_pct = FUTURES_MAX_PORTFOLIO_MARGIN_PCT if is_futures else MAX_PORTFOLIO_MARGIN_PCT
    available_margin_budget = (equity * budget_pct) - current_margin_used
...
def manage_active_position(key, current_price):
    ...
    with state_lock:
        ...
        regime = pos["regime"]
        grade = pos.get("grade", "")
        pos_market_type = pos.get("market_type", market_type_by_venue.get(ex_name, "SPOT"))
    ...
    if exit_triggered:
        ...
        try:
            ...
            log_trade_to_ledger(
                ...
                market_type=pos_market_type, grade=grade, pnl=pnl
            )
            ...
            save_state()
            # Freeze this market from immediate re-entry and reset any
            # signal streak — this is what stops the bot whipsawing back
            # into the same symbol right after closing it.
            start_symbol_cooldown(key)
            clear_signal_confirmation(key)
...
def execution_orchestrator():
        ...
        for symbol in pair_list:
            ...
            if score > 0.0:
                ...
                scored_candidates.append({
                    ...
                    'market_type': market_type_by_venue.get(ex_name, 'SPOT'),
                    'regime': regime,
                    'score': score,
                    'grade': grade,
                    'price': metrics['close'],
                ...
                # Signal died — drop any confirmation streak so a later
                # revival must earn confirmations fresh (no stale carryover).
                clear_signal_confirmation(composite_key)
    ...
    for candidate in scored_candidates:
        ...
        # --- Risk Gate 1: Trade Quality Grade -------------------------
        if not grade_passes_minimum(grade):
            continue

        # --- Risk Gate 2: Symbol Cooldown (anti to-and-fro) ------------
        in_cooldown, cooldown_until = is_symbol_in_cooldown(key)
        if in_cooldown:
            print(f"🧊 [{key}] Skipped — cooling down until {cooldown_until} after recent close.")
            continue

        # --- Risk Gate 3: Signal Confirmation (anti flip-flop) ---------
        confirmed, confirmations_so_far = register_signal_and_check_confirmed(key, regime, score)
        if not confirmed:
            print(f"⏳ [{key}] Signal building conviction: {confirmations_so_far}/{MIN_SIGNAL_CONFIRMATIONS} confirmations (Grade {grade}).")
            continue

        print(f"🎯 Top Venue Setup Selected: [{key}] | Regime: {regime} | Grade: {grade} | Score: {score:.3f}")
        ...
        try:
            ...
            if PAPER_TRADING:
                ...
                if plan['leverage'] > 1.0:
                    if candidate['market_type'] == 'FUTURES':
                        try:
                            client.set_leverage(int(plan['leverage']), sym)
                        except Exception as lev_err:
                            print(f"⚠️ [{key}] set_leverage call failed (continuing with venue default): {lev_err}")
                    elif ex_name == "kraken":
            ...
            msg = (
                f"🟢 *{ex_name.upper()} SCANNER ENTRY ({regime})* [{candidate['market_type']}]\n"
                f"Market: `{key}`\n"
                f"Trade Grade: `{grade}`  |  Score: `{score:.3f}`\n"
            ...
            log_trade_to_ledger(
                ...
                market_type=candidate['market_type'], grade=grade
            )
...
<head>
  ...
  <!-- Add-to-Home-Screen on iPhone 17 Pro Max / iPad Pro M4 launches this as a
       standalone full-screen app (no Safari chrome) via the meta tags above. -->
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .type-tag.spot { background: rgba(88, 166, 255, 0.15); color: var(--blue); border: 1px solid var(--blue); }
    .type-tag.futures { background: var(--amber-glow); color: var(--amber); border: 1px solid var(--amber); }
    .grade-badge {
      display: inline-flex;
      align-items: center;
    ...
    .grade-a { background: var(--green-glow); color: var(--green); border: 1px solid var(--green); }
    .grade-b { background: rgba(88, 166, 255, 0.15); color: var(--blue); border: 1px solid var(--blue); }
    .grade-c { background: var(--amber-glow); color: var(--amber); border: 1px solid var(--amber); }
    .grade-d { background: var(--red-glow); color: var(--red); border: 1px solid var(--red); }
    .live-dot {
    ...
    /* =================================================================
       IPAD PRO M4 LAYOUT (11" portrait 834px / landscape 1194px,
       13" portrait 1032px / landscape 1366px) — tablet-tuned multi-column
       ================================================================= */
    @media (min-width: 768px) and (max-width: 1400px) {
      ...
      .section-title { font-size: 15px; margin-top: 20px; }
      .param-grid { grid-template-columns: repeat(4, 1fr); }
      .live-trade-card { grid-template-columns: repeat(4, 1fr); }
      table { font-size: 13px; }
      th, td { padding: 10px 12px; }
    }

    /* iPad Pro landscape specifically: room for a two-column layout of
       positions + opportunities side by side */
    @media (min-width: 1180px) {
      ...
      .tablet-split .table-container { margin-bottom: 0; }
    }
  </style>
</head>
<body>
  ...
  <div class="table-container responsive-table">
    <table>
      ...
      <tbody id="active-positions-tbody">
        <tr><td colspan="13" style="color: var(--muted); text-align: center;">No open positions. Dual scanner evaluating candidate markets.</td></tr>
      </tbody>
    </table>
  </div>
  ...
  <div class="table-container responsive-table">
    <table>
      ...
      <tbody id="opportunities-tbody">
        <tr><td colspan="10" style="color: var(--muted); text-align: center;">Scanning candidate markets across Binance & Kraken...</td></tr>
      </tbody>
    </table>
  </div>

  <div class="section-title">📜 Trade History (Live Ledger)</div>
  <div class="table-container" style="padding: 14px;">
    <div style="display:flex; justify-content: space-between; align-items:center; flex-wrap: wrap; gap: 10px; margin-bottom: 10px;">
      <span style="color: var(--muted); font-size: 12px;">Every open &amp; close is documented here and mirrored to the CSV ledger.</span>
      <a href="/history.csv" style="background: var(--card); border: 1px solid var(--border); color: var(--blue); padding: 8px 14px; border-radius: 8px; font-size: 12px; text-decoration: none; font-weight: 600;">⬇ Download CSV</a>
    </div>
  </div>
  <div class="table-container responsive-table">
    <table>
      ...
      <tbody id="history-tbody">
        <tr><td colspan="12" style="color: var(--muted); text-align: center;">Loading trade history...</td></tr>
      </tbody>
    </table>
  </div>
  ...
  <script>
    ...
    function venueClass(ex) {
      return (ex || '').startsWith('binance') ? 'venue-binance' : 'venue-kraken';
    }

    function matchesFilter(ex) {
      return venueFilter === 'all' || (ex || '').startsWith(venueFilter);
    }
    ...
    function renderDashboard(data) {
      ...
      if (posEntries.length === 0) {
        actBody.innerHTML = '<tr class="empty-row"><td colspan="13" class="empty-cell" style="color: var(--muted); text-align: center;">No open positions for this venue filter.</td></tr>';
      } else {
        for (const [key, pos] of posEntries) {
          ...
          row.innerHTML = `
            ...
            <td data-label="Type"><span class="type-tag ${(pos.market_type||'SPOT').toLowerCase()}">${pos.market_type || 'SPOT'}</span></td>
            <td data-label="Symbol"><strong>${pos.symbol}</strong></td>
            <td data-label="Regime">${pos.regime}</td>
            <td data-label="Grade"><span class="grade-badge grade-${(pos.grade||'-').toLowerCase()}">${pos.grade || '-'}</span></td>
      ...
      if (opps.length === 0) {
        oppBody.innerHTML = '<tr class="empty-row"><td colspan="10" class="empty-cell" style="color: var(--muted); text-align: center;">Evaluating candidate markets across venues...</td></tr>';
      } else {
        for (const item of opps) {
          ...
          row.innerHTML = `
            ...
            <td data-label="Type"><span class="type-tag ${(item.market_type||'SPOT').toLowerCase()}">${item.market_type || 'SPOT'}</span></td>
            <td data-label="Market"><strong>${item.symbol}</strong></td>
            <td data-label="Regime">${item.regime}</td>
            <td data-label="Grade"><span class="grade-badge grade-${(item.grade||'-').toLowerCase()}">${item.grade || '-'}</span></td>
    ...
    async function updateHistory() {
      try {
        ...
        if (rows.length === 0) {
          body.innerHTML = '<tr class="empty-row"><td colspan="12" class="empty-cell" style="color: var(--muted); text-align: center;">No trades logged yet.</td></tr>';
          return;
        }
        body.innerHTML = '';
        for (const r of rows) {
          const exClass = venueClass((r.Exchange || '').toLowerCase());
          const pnlVal = (r.PnL === '' || r.PnL === undefined) ? null : Number(r.PnL);
          const pnlCell = pnlVal === null ? '--' : `<span class="${pnlClass(pnlVal)}">${fmtMoney(pnlVal)}</span>`;
          const row = document.createElement('tr');
          row.innerHTML = `
            <td data-label="Time">${(r.Timestamp || '').replace('T', ' ').slice(0, 19)}</td>
            <td data-label="Venue"><span class="venue-tag ${exClass}">${r.Exchange || ''}</span></td>
            <td data-label="Type"><span class="type-tag ${(r.MarketType||'SPOT').toLowerCase()}">${r.MarketType || 'SPOT'}</span></td>
            <td data-label="Symbol"><strong>${r.Symbol || ''}</strong></td>
            <td data-label="Side">${r.Side || ''}</td>
            <td data-label="Regime">${r.MarketRegime || ''}</td>
            <td data-label="Grade"><span class="grade-badge grade-${(r.Grade||'-').toLowerCase()}">${r.Grade || '-'}</span></td>
            <td data-label="Price">$${Number(r.ExecutionPrice || 0).toFixed(2)}</td>
            <td data-label="Amount">${r.Amount || ''}</td>
            <td data-label="Leverage">${r.Leverage || 1}x</td>
            <td data-label="PnL">${pnlCell}</td>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    .grade-a { background: rgba(46, 160, 67, 0.18); color: #2ea043; }
    .grade-b { background: rgba(88, 166, 255, 0.18); color: #58a6ff; }
    .grade-c { background: rgba(210, 153, 34, 0.18); color: #d29922; }
    .grade-d { background: rgba(248, 81, 73, 0.18); color: #f85149; }
    ...
    @media (min-width: 600px) {
      .tg-pos-grid { grid-template-columns: repeat(4, 1fr); }
    }
...
<body>
  ...
  <script>
    ...
    function venueClass(ex) { return (ex || '').startsWith('binance') ? 'venue-binance' : 'venue-kraken'; }
        ...
        for (const [key, pos] of posEntries) {
          ...
          card.innerHTML = `
            <div class="tg-pos-top">
              <div class="tg-pos-symbol"><span class="venue-pill ${venueClass(pos.exchange)}">${pos.exchange}</span> ${pos.symbol} <span class="grade-pill grade-${(pos.grade||'-').toLowerCase()}">${pos.grade || '-'}</span></div>
              <div class="tg-pos-pnl ${pnlClass(pos.unrealized_pnl)}">${fmtMoney(pos.unrealized_pnl)}</div>
            </div>
            <div class="tg-pos-grid">
              <div><span>Type</span>${pos.market_type || 'SPOT'}</div>
        ...
        for (const item of opps.slice(0, 8)) {
          ...
          row.innerHTML = `
            <div class="tg-opp-left">
              ...
              <span class="grade-pill grade-${(item.grade||'-').toLowerCase()}">${item.grade || '-'}</span>
            </div>
...
def config():
    return jsonify({
        ...
        "futures_enabled": ENABLE_FUTURES,
        "futures_max_leverage": FUTURES_MAX_LEVERAGE if ENABLE_FUTURES else None,
        "futures_max_portfolio_margin_pct": FUTURES_MAX_PORTFOLIO_MARGIN_PCT if ENABLE_FUTURES else None,
        "min_grade_to_trade": MIN_GRADE_TO_TRADE,
        "min_signal_confirmations": MIN_SIGNAL_CONFIRMATIONS,
        "symbol_cooldown_minutes": SYMBOL_COOLDOWN_MINUTES,
        "venues": {k: v for k, v in market_type_by_venue.items()}
    }), 200
...
def history():
    """Recent trade history as JSON for the dashboard's live history table."""
    ...
    try:
        ...
        return jsonify({"error": f"Failed to read ledger: {e}", "records": []}), 500
    return jsonify({"count": len(records), "records": records}), 200

@app.route("/history.csv", methods=["GET"])
def history_csv():
    """Full trade ledger download, exactly what you'd import into a
    spreadsheet — every open/close is documented here in order."""
    if not os.path.isfile(CSV_FILE_PATH):
    ...
    return send_file(
        ...
        mimetype="text/csv",
        as_attachment=True,
        download_name=f"trade_history_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
    )
...
if __name__ == "__main__":
    ...
    print("🗂️  Trade History (JSON): /history")
    print("🗂️  Trade History (CSV download): /history.csv")
