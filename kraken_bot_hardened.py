...
def save_state():
    """Persist all positions atomically to Upstash Redis REST and local sidecar."""
    with state_lock:
        state = {
            'positions': positions,
            'consecutive_failures': consecutive_failures,
...
def load_state():
    ...
    global positions, consecutive_failures
    ...
    try:
        ...
        if recovered_raw:
            print("🌐 Recovered multi-pair state from Upstash Serverless Redis.")
    except Exception as e:
    ...
    try:
        ...
        print(f"♻️ Recovered Multi-Pair State (last_updated={state.get('last_updated')})")
        for sym in SYMBOLS:
            p = positions[sym]
...
def reconcile_state_with_exchange():
    """Cross-check recovered state for all symbols against live Kraken balances."""
    global positions
    ...
    try:
        ...
        print("✅ Startup multi-pair reconciliation completed successfully.")
        return True
...
def log_trade_to_ledger(timestamp, symbol, order_id, side, regime, price, amount, stop_loss, status):
    ...
    try:
        ...
        print(f"🌐 Trade pushed to Upstash ledger list '{REDIS_LEDGER_KEY}' [{symbol}]")
    except Exception as e:
...
def check_safety_preflight():
    ...
    try:
        balances = exchange.fetch_balance()
        usdt_balance = balances['free'].get('USDT', 0.0)

        if usdt_balance < MIN_BALANCE_USDT:
            msg = f"❌ *EMERGENCY SHUTDOWN*: Free USDT balance ({usdt_balance:.2f}) fell below floor ({MIN_BALANCE_USDT:.2f})!"
            send_telegram_notification(msg)
            raise SystemExit(msg)
...
def manage_active_position(symbol, current_price):
    ...
    print(
        f"🛡️ [{symbol}] Active Position | Entry: {curr_entry:.2f} | "
    ...
    if current_price <= curr_stop:
        ...
        try:
            ...
            msg = f"🚨 *CRITICAL EXIT* [{symbol}]: Trailing stop hit at `{current_price:.2f}`. Position closed."
            send_telegram_notification(msg)
            log_trade_to_ledger(
...
def execution_orchestrator():
    ...
    for symbol in SYMBOLS:
        ...
        print(f"🔍 [{symbol}] Scanning Entries | Price: {current_price:.2f} | ADX: {metrics['adx']:.1f}")
...
def trading_engine_loop():
    ...
    try:
        ...
        print(f"🚀 Hardened Multi-Shield Engaged. Monitoring {len(SYMBOLS)} symbols.")
        send_telegram_notification(f"🚀 Kraken Bot Multi-Pair Engine Active! Monitoring: {', '.join(SYMBOLS)}")
        ...
        update_engine_status(
            ...
            last_error=error_details,
            halt_reason="Trading engine startup or reconciliation failed.",
            thread_alive=False,
            startup_reconciled=False
        )
